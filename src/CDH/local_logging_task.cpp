...
#include "local_logging_task.hpp"
...
void logData(const local_logging::SensorLog& data) {
    // Write data to flash memory
    const char *filename = "datalog.bin";

    if (!SerialFlash.exists(filename)) {
        if (!SerialFlash.create(filename, sizeof(local_logging::SensorLog) * 10000)) { // Space for 10 000 logs
    ...
    if (!flashFile) {
        ...
        // write data to flash
        if (flashFile.write(&data, sizeof(local_logging::SensorLog)) != sizeof(local_logging::SensorLog)) {
...
void datalogger_thread(void*) {
    ...
    while (true) {
        ...
        // Read sensor data
        uint32_t currentTimestamp = pdTICKS_TO_MS(xTaskGetTickCount());

        local_logging::SensorLog currentLog;

...
void att_est_predict_thread(void*) {
    ...
    while (1) {
        ...
        local_mcu_state = mcu_state.load();
        if (local_mcu_state == ControllerState::LV_ON || local_mcu_state == ControllerState::CALIBRATING) continue;
        if (xSemaphoreTake(att_est_mutex, pdMS_TO_TICKS(8)) == pdTRUE && // TODO: change delay to match freq of gyro
...
void att_est_update_thread(void*) {
    ...
    while (1) {
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        local_mag_data = mag_data;
        local_mcu_state = mcu_state.load();
        if (local_mcu_state == ControllerState::LV_ON || local_mcu_state == ControllerState::CALIBRATING) continue;
        if (last_action_was_predict) {
            if (xSemaphoreTake(att_est_mutex, pdMS_TO_TICKS(23)) == pdTRUE && // TODO: change delay to match freq of mag
            att_estimator.initialized) {
                // Get mag data
                long long current_time_us = pdTICKS_TO_US(xTaskGetTickCount());
                att_estimator.update_mag(local_mag_data.toVector());

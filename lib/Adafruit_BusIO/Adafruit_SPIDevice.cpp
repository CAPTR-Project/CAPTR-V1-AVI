...
bool Adafruit_SPIDevice::begin(void) {
  ...
  if (_spi) { // hardware SPI
    ...
    pinMode(_sck, arduino::OUTPUT);

    if ((_dataMode == SPI_MODE0) || (_dataMode == SPI_MODE1)) {
      // idle low on mode 0 and 1
      digitalWrite(_sck, arduino::LOW);
    } else {
      // idle high on mode 2 or 3
      digitalWrite(_sck, arduino::HIGH);
    }
    if (_mosi != -1) {
      pinMode(_mosi, arduino::OUTPUT);
      digitalWrite(_mosi, arduino::HIGH);
    }
    if (_miso != -1) {
      pinMode(_miso, arduino::INPUT);
    }
  }

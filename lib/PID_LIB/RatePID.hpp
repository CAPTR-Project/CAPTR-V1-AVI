...
{
public:
    RatePID(float dt, float max, float min, Eigen::Vector3d Kp, Eigen::Vector3d Ki, Eigen::Vector3d Kd, Eigen::Vector3d integ_clamp, float alpha, float tau)
    : pidX_(dt, max, min, Kp(0), Ki(0), Kd(0), integ_clamp(0), alpha, tau),
    pidY_(dt, max, min, Kp(1), Ki(1), Kd(1), integ_clamp(1), alpha, tau),
    pidZ_(dt, max, min, Kp(2), Ki(2), Kd(2), integ_clamp(2), alpha, tau) {}
